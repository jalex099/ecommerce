      ...
      <AccordionSummary>
        <Box className="flex row items-center justify-around gap-2 py-2 px-4 rounded-md w-full cursor-pointer">
          ...
          <Box className={"flex-1 flex justify-start items-center"}>
            <Regular14 className={" text-center"}>
              {paymentMethod?.label}
            </Regular14>
          </Box>
          <Radio
      ...
      <AccordionDetails>
            ...
            {checkoutState?.isFinishedAddedCard && checkoutState?.cardNumber ? (
              <Box className={"flex flex-row justify-between gap-4"}>
                <Box className={"flex flex-col gap-2"}>
                  <SemiBold12>Tarjeta seleccionada</SemiBold12>
                  <Regular12>{checkoutState?.showedCardNumber}</Regular12>
                </Box>
                <Button
                  variant="text"
                  color="primary"
        ...
        {paymentMethod?.code === "TRANSFER" && (
          ...
          >
            ...
            {parameters
              ?.filter(({ name }) => name === "BANK_ACCOUNT_NUMBER")
              ?.map((parameter, index) => (
                <BankAccount key={parameter?._id} bankAccount={parameter} />
              ))}
            <Regular12
              className={"text-center"}
              styles={{ color: (theme) => theme?.palette?.opacity40?.main }}
            >
              Mantenga presionado el n&uacute;mero de cuenta para copiarlo
            </Regular12>
            <Regular12 className={"opacity-60 pt-6"}>
              * Pago con transferencia bancaria se realiza la
              confirmaci&oacute;n en un plazo m&aacute;ximo de 24 horas
            </Regular12>
          </motion.div>
        )}
      </AccordionDetails>
